...
public partial class PeFile
{
    ...
    public void AddImports(List<AdditionalImport> additionalImports)
    {
        ...
        {
            return ImageSectionHeaders.First(sh => sh.VirtualAddress + sh.VirtualSize >= importRva);
        }

        int EstimateAdditionalNeededSpace()
        {
            return additionalImports.Select(ai => ai.Functions).Count() * 64;
        }
        ...
        ImageNtHeaders.OptionalHeader.DataDirectory[(int)DataDirectoryType.Import].Size =
            (uint)(importSize + sizeOfImpDesc * numAddImpDescs);
