        ...
        _index = new Dictionary<Index, IMetaDataIndex>
        {
            ...
            // Coded Indices
            {
                ...
                new CodedIndex(tables, (byte)MetadataToken.TypeDef, (byte)MetadataToken.TypeReference,
                    (byte)MetadataToken.TypeSpecification)
            },
            {
                Index.HasConstant,
                new CodedIndex(tables, (byte)MetadataToken.Field, (byte)MetadataToken.Parameter,
                    (byte)MetadataToken.Property)
            },
            {
                Index.HasCustomAttribute, new CodedIndex(tables,
                    (byte)MetadataToken.MethodDef, (byte)MetadataToken.Field, (byte)MetadataToken.TypeReference,
                    (byte)MetadataToken.TypeDef,
                    (byte)MetadataToken.Parameter, (byte)MetadataToken.InterfaceImplementation,
                    (byte)MetadataToken.MemberReference, (byte)MetadataToken.Module,
                    (byte)MetadataToken.DeclarativeSecurity, (byte)MetadataToken.Property, (byte)MetadataToken.Event,
                    (byte)MetadataToken.StandAloneSignature,
                    (byte)MetadataToken.ModuleReference, (byte)MetadataToken.TypeSpecification,
                    (byte)MetadataToken.Assembly, (byte)MetadataToken.AssemblyReference,
            ...
            { Index.HasFieldMarshal, new CodedIndex(tables, (byte)MetadataToken.Field, (byte)MetadataToken.Parameter) },
            {
                ...
                new CodedIndex(tables, (byte)MetadataToken.TypeDef, (byte)MetadataToken.MethodDef,
                    (byte)MetadataToken.Assembly)
            },
            {
                Index.MemberRefParent,
                new CodedIndex(tables, (byte)MetadataToken.TypeDef, (byte)MetadataToken.TypeReference,
                    (byte)MetadataToken.ModuleReference, (byte)MetadataToken.MethodDef,
                    (byte)MetadataToken.TypeSpecification)
            },
            { Index.HasSemantics, new CodedIndex(tables, (byte)MetadataToken.Event, (byte)MetadataToken.Property) },
            {
                Index.MethodDefOrRef,
                new CodedIndex(tables, (byte)MetadataToken.MethodDef, (byte)MetadataToken.MemberReference)
            },
            { Index.MemberForwarded, new CodedIndex(tables, (byte)MetadataToken.Field, (byte)MetadataToken.MethodDef) },
            {
                Index.Implementation,
                new CodedIndex(tables, (byte)MetadataToken.Field, (byte)MetadataToken.AssemblyReference,
                    (byte)MetadataToken.ExportedType)
            },
            {
                Index.CustomAttributeType,
                new CodedIndex(tables, Unused, Unused, (byte)MetadataToken.MethodDef,
                    (byte)MetadataToken.MemberReference, Unused)
            },
            {
                Index.ResolutionScope,
                new CodedIndex(tables, (byte)MetadataToken.Module, (byte)MetadataToken.ModuleReference,
                    (byte)MetadataToken.AssemblyReference, (byte)MetadataToken.TypeReference)
            },
            {
