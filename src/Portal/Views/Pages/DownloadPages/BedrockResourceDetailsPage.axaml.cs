...
public partial class BedrockResourceDetailsPage : UserControl, ITioTabPage
{
    ...
    public BedrockResourceDetailsPage() : this(new JavaResourceDetailsTarget(BedrockResourceDefinitions.BehaviorPack,
    ...
    {
        InitializeComponent();
        ViewModel = new BedrockResourceDetailsPageViewModel(target);
        DataContext = ViewModel;
        ViewModel.TargetVersionGroupReady += group =>
        {
            _targetVersionGroup = group;
            LayoutUpdated += OnLayoutUpdated;
        };
        PageInfo = new PageInfo
            { Title = $"{target.Definition.DisplayName}详情", Icon = StreamGeometry.Parse(JavaResourceDetailsIcon.Data) };
    ...
    public static void Open(TopLevel sender, JavaResourceDetailsTarget target, string title)
    {
        ...
        var tab = new TabEntry(window, new BedrockResourceDetailsPage(target), title: title);
