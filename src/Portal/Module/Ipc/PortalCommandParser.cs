...
{
    /// <summary>参数不是 Portal 命令，按普通启动处理。</summary>
    NotACommand,
    Help,
    Error,
    Command
...
/// <summary>
/// 把命令行参数或 portal:// URI 解析成 <see cref="PortalCommand"/>。
/// 两种入口等价：
///   Portal.Desktop.exe install vanilla 1.21.8 --folder B --id my-1.21.8
...
public static class PortalCommandParser
{
    ...
    /// <summary>
    /// 解析 portal:// URI。浏览器传参时整个 URI 是一个参数且经过百分号编码；
    /// 这里手动解析，兼容 portal://launch?... / portal://launch/?... / portal:launch?... 等形式。
    /// </summary>
    ...
    private static List<KeyValuePair<string, string>> ParseQuery(string query)
    {
        ...
        foreach (var pair in query.Split('&', StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            // '+' 号不按空格处理：实例 ID（如“1.21.8 Fabric + OptiFine”）里的加号是字面量，空格应编码为 %20。
