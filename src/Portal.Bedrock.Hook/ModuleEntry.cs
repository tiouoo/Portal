...
/// 与显式导出 HookInit（由 Portal.Preload 在 LoadLibraryW 后调用），
/// 两者均只触发一次，确保工作线程被拉起。
/// </summary>
internal static unsafe class ModuleEntry
{
    ...
    private static nint GetSelfModuleBase()
    {
        try
        {
            return NativeMethods.GetModuleHandleW("Portal.XUserHook.dll");
        }
        catch
        {
            return 0;
    ...
    private static uint WorkerThread(nint parameter)
    {
        ...
        XUserBridge.Info("Portal.XUserHook worker started");
        XUserBridge.Info($"Portal.XUserHook base=0x{GetSelfModuleBase():X} pid={Environment.ProcessId}");
